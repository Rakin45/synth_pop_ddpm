...
class TransformerWithPositionalEncoding(nn.Module):
    def __init__(self, feature_size=8, num_layers=3, dropout=0.1, sequence_length=144):
        super().__init__()
        self.feature_size = feature_size
        self.time_embedding = nn.Embedding(sequence_length, feature_size)

        self.pos_encoder = PositionalEncoding(feature_size)
        self.encoder_layer = nn.TransformerEncoderLayer(
            d_model=feature_size, nhead=8, dropout=dropout, batch_first=True
        )
    ...
    def forward(self, src, timesteps):
        ...
        assert (
            timesteps.max() <= self.time_embedding.num_embeddings
        ), "Timestep exceeds sequence length."
        assert (
            timesteps.min() >= 1
        ), "Timestep is below 1."  # Adjust based on your actual timestep range
        time_encodings = self.time_embedding(
            timesteps
        )  # Shape: [batch_size, feature_size]
        # print("Time encodings shape:", time_encodings.shape)

        # Ensure time_encodings is broadcastable to src's shape
        time_encodings = time_encodings.unsqueeze(
            1
        )  # Add sequence length dim: [batch_size, 1, feature_size]
        # print("Time encodings shape after unsqueeze:", time_encodings.shape)
        time_encodings = time_encodings.expand(
            -1, src.size(1), -1
        )  # Expand to match src: [batch_size, sequence_length, feature_size]
